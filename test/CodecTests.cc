...
{
    std::std::unique_ptr<InputStream> in = memoryInputStream(os);
    const char *b;
    size_t n;
...
static std::unique_ptr<OutputStream> generate(Encoder& e, const char* calls,
    const vector<string>& values)
{
    Scanner sc(calls);
    vector<string>::const_iterator it = values.begin();
    std::unique_ptr<OutputStream> ob = memoryOutputStream();
    e.init(*ob);
...
void testEncoder(const EncoderPtr& e, const char* writerCalls,
    vector<string>& v, std::unique_ptr<OutputStream>& p)
{
...
void testCodec(const TestData& td) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        for (unsigned int i = 0; i <= td.depth; ++i) {
            ...
            std::unique_ptr<InputStream> in = memoryInputStream(*p);
...
void testGeneric(const TestData& td) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        ...
        std::unique_ptr<InputStream> in1 = memoryInputStream(*p);
        d1->init(*in1);
        ...
        BOOST_TEST_CHECKPOINT("Test: " << testNo << ' '
            << " schema: " << td.schema
            << " calls: " << td.calls);
        std::unique_ptr<InputStream> in2 = memoryInputStream(*ob);
...
void testGenericResolving2(const TestData4& td) {
    ...
    std::unique_ptr<OutputStream> p = generate(*CodecFactory::newEncoder(wvs),
        td.writerCalls, wd);
    // dump(*p);
    DecoderPtr d1 = CodecFactory::newDecoder(wvs);
    std::unique_ptr<InputStream> in1 = memoryInputStream(*p);
